...
_YAML_ARTICLES = [
    # ── Outlook granular subtypes ──────────────────────────────────
    {
        ...
        "keywords": ["not receiving", "no new emails", "missing emails", "emails not arriving", "stopped receiving"],
        "ownership_group": "endpoint-productivity",
        "symptoms": ["New mail is not arriving in the inbox", "Mailbox appears frozen on old messages"],
        "probable_causes": ["Work Offline mode is enabled", "Inbox rule moving or deleting mail", "Sync hiccup with Exchange"],
        "resolution_steps": [
            {"step_number": 1, "instruction": "Make sure Work Offline is turned off", "details": "Send/Receive tab — if 'Work Offline' is highlighted, click it to go back online."},
            {"step_number": 2, "instruction": "Force a manual Send/Receive", "details": "Press F9 or click Send/Receive All Folders and watch for new mail."},
            {"step_number": 3, "instruction": "Check Inbox rules aren't moving or deleting mail", "details": "File > Manage Rules & Alerts. Disable any rule that moves matching mail to another folder or deletes it."},
            {"step_number": 4, "instruction": "Check the Junk Email and other folders", "details": "Incoming mail may be filtered into Junk Email. Mark legitimate senders as 'Not Junk'."},
        ],
        "validation_steps": [{"step_number": 1, "instruction": "Ask a colleague to send a test email and confirm it arrives within 2 minutes."}],
    ...
    },
    {
        ...
        "probable_causes": ["Work Offline mode enabled", "Oversized attachment blocking the queue", "Network or Exchange connection issue"],
        "resolution_steps": [
            {"step_number": 1, "instruction": "Confirm Outlook is online (Work Offline is off)", "details": "Send/Receive tab — ensure 'Work Offline' is not highlighted."},
            {"step_number": 2, "instruction": "Open the Outbox and check for an oversized message", "details": "A message with a very large attachment can block the queue. Remove or shrink the attachment."},
            {"step_number": 3, "instruction": "Force a Send/Receive", "details": "Press F9 to retry sending the queued messages."},
        ],
        "validation_steps": [{"step_number": 1, "instruction": "Send a test message to yourself and verify it leaves the Outbox and arrives in your Inbox."}],
    ...
    },
    {
        ...
        "ownership_group": "endpoint-productivity",
        "symptoms": ["Outlook freezes or becomes unresponsive", "Takes a long time to open or switch folders"],
        "probable_causes": ["Heavy COM add-ins", "Large local data file", "Pending Office updates"],
        "resolution_steps": [
            {"step_number": 1, "instruction": "Disable non-essential COM add-ins", "details": "File > Options > Add-ins > Manage: COM Add-ins > Go. Untick add-ins you don't need, then restart Outlook."},
            {"step_number": 2, "instruction": "Restart Outlook (and the PC if needed)", "details": "Fully close Outlook and reopen. A reboot clears background processes that can slow it down."},
            {"step_number": 3, "instruction": "Apply pending Office updates", "details": "File > Office Account > Update Options > Update Now."},
        ],
        "validation_steps": [{"step_number": 1, "instruction": "Open a large folder and verify Outlook responds immediately without hanging."}],
    ...
    },
    {
        ...
        "ownership_group": "endpoint-productivity",
        "symptoms": ["Status bar shows 'Working Offline' or 'Disconnected'", "Outlook does not sync"],
        "probable_causes": ["Work Offline mode accidentally enabled", "VPN not connected", "Network issue"],
        "resolution_steps": [
            {"step_number": 1, "instruction": "Turn off Work Offline", "details": "Send/Receive tab > click 'Work Offline' so it is no longer highlighted."},
            {"step_number": 2, "instruction": "Confirm you have network connectivity", "details": "Open a browser and load any internal site. Connect VPN if your mail requires it."},
            {"step_number": 3, "instruction": "Restart Outlook", "details": "Close and reopen Outlook; the status bar should show 'Connected'."},
        ],
        "validation_steps": [{"step_number": 1, "instruction": "Confirm the status bar shows 'Connected' and a new email arrives within 2 minutes of sending yourself a test."}],
    ...
    },
    {
        ...
        "resolution_steps": [
            {"step_number": 1, "instruction": "Restart Outlook", "details": "Fully close and reopen the app."},
            {"step_number": 2, "instruction": "Confirm you are online and connected to VPN if required", "details": "Check the status bar shows 'Connected'."},
            {"step_number": 3, "instruction": "Apply pending Office updates", "details": "File > Office Account > Update Options > Update Now."},
        ],
    ...
    # ── Email alias / shared mailbox ──────────────────────────────
    {
        ...
        "keywords": ["shared mailbox", "can't access shared mailbox", "alias not working", "distribution list", "send as", "mailbox not showing"],
        "ownership_group": "endpoint-productivity",
        "symptoms": ["Shared mailbox not visible in Outlook", "Cannot send from an alias address", "Access Denied on shared inbox"],
        "probable_causes": ["Permissions not yet propagated (can take up to 60 min)", "Access not yet granted — IT ticket required"],
        "resolution_steps": [
            {"step_number": 1, "instruction": "Close Outlook and reopen it to force a permissions refresh", "details": "M365 permissions can take up to 60 minutes to propagate after IT grants access."},
            {"step_number": 2, "instruction": "Remove and re-add the shared mailbox in Outlook", "details": "Go to File > Account Settings > Account Settings > select your main account > Change > More Settings > Advanced tab. Remove the shared mailbox, click OK, reopen Outlook, then add it again using its full email address."},
            {"step_number": 3, "instruction": "Open the shared mailbox via Outlook on the Web", "details": "At outlook.office.com, click your profile picture > Open another mailbox. Type the shared mailbox address. This confirms whether access has been granted at all."},
            {"step_number": 4, "instruction": "If you do not have access, raise an IT ticket with manager approval", "details": "Raise a ticket: 'Shared Mailbox Access — [mailbox name] — [your name]' and CC your manager. IT will action within 1 business day."},
        ],
    ...
    },
    {
        ...
        "keywords": ["alias", "email alias", "add alias", "create alias", "remove alias", "secondary email", "smtp"],
        "ownership_group": "endpoint-productivity",
        "symptoms": ["Need a secondary email address added to my account", "Team needs a new alias created or an existing one removed"],
        "probable_causes": ["Alias changes are IT-managed and require a ticket"],
        "resolution_steps": [
            {"step_number": 1, "instruction": "Raise an IT ticket for the alias change", "details": "In Freshservice raise a ticket titled: 'Alias Request — [Add/Remove/Create] — [your name]'. Include the exact alias email address required."},
            {"step_number": 2, "instruction": "Specify the alias format: firstname.lastname@aditiconsulting.com", "details": "Non-standard aliases (team or role aliases) require business justification and manager approval in the ticket."},
            {"step_number": 3, "instruction": "Wait up to 1 business day for IT to implement the change", "details": "IT processes alias changes within 1 business day. You will receive a confirmation email when done."},
            {"step_number": 4, "instruction": "Test the new alias by sending a test email to it", "details": "After IT confirms, ask a colleague to send a test email to the alias. Mention 'Send As' permission in the original ticket if you also need to send from the alias."},
        ],
        "validation_steps": [{"step_number": 1, "instruction": "Ask a colleague to send a test email to the new alias and confirm it arrives in your primary inbox."}],
    ...
    # ── Hardware audio ─────────────────────────────────────────────
    {
        ...
        "severity_hint": "high",
        "tags": ["audio", "voice breaks", "cutting out", "interview", "call quality", "teams", "zoom"],
        "keywords": ["voice breaks", "audio cutting", "cutting out", "breaking up", "choppy audio", "robotic voice", "audio drops"],
        "ownership_group": "endpoint-productivity",
        "symptoms": ["Voice breaking up or cutting out on calls", "Participants report choppy or robotic audio"],
        "probable_causes": ["Too many open browser tabs consuming bandwidth/CPU", "Wrong audio device selected", "Loose headset connection"],
        "resolution_steps": [
            {"step_number": 1, "instruction": "Close all unused browser tabs and background applications", "details": "Each open tab and app consumes bandwidth and CPU. Close everything not needed for the call — this is the single most common fix for choppy audio."},
            {"step_number": 2, "instruction": "In Teams/Zoom, go to Settings > Audio and select your headset as both mic and speaker", "details": "Teams: click '...' > Settings > Devices. Zoom: Settings > Audio. Explicitly select your headset from the dropdown — do not leave it on 'Default'."},
            {"step_number": 3, "instruction": "Unplug and firmly replug your headset; try a different USB port", "details": "A loose connection causes intermittent audio. Unplug, wait 5 seconds, and reconnect. If USB, try a different port."},
            {"step_number": 4, "instruction": "Mute and unmute yourself to reset the audio stream", "details": "Click the mute button in Teams/Zoom and wait 2 seconds, then unmute. This resets the audio capture pipeline."},
            {"step_number": 5, "instruction": "Rejoin the call if the issue continues", "details": "Leave the meeting, close Teams/Zoom fully (system tray), reopen, and rejoin. A fresh session clears any corrupted audio state."},
        ],
        "validation_steps": [{"step_number": 1, "instruction": "Ask the other participant to confirm they can hear you clearly with no breaks."}],
    ...
    },
    {
        ...
        "keywords": ["candidate cannot hear", "they can't hear me", "mic not working", "microphone not detected", "no microphone", "muted"],
        "ownership_group": "endpoint-productivity",
        "symptoms": ["Other person on the call cannot hear me", "Microphone icon shows no activity in Teams/Zoom"],
        "probable_causes": ["Microphone muted in app or on headset", "Wrong microphone device selected", "Windows microphone privacy blocking the app"],
        "resolution_steps": [
            {"step_number": 1, "instruction": "Check you are not muted in Teams/Zoom", "details": "Look for the microphone icon in the call toolbar — a red slash means you are muted. Click it to unmute. Also check the physical mute button on your headset."},
            {"step_number": 2, "instruction": "Select the correct microphone in Teams/Zoom settings", "details": "Teams: '...' > Settings > Devices > Microphone — pick your headset microphone. Zoom: Settings > Audio > Microphone. Do not use 'Default' if multiple devices are listed."},
            {"step_number": 3, "instruction": "Check Windows microphone privacy settings", "details": "Settings > Privacy & Security > Microphone. Ensure 'Microphone access' is ON and that the app (Teams, Zoom, Chrome) is allowed."},
            {"step_number": 4, "instruction": "Close other apps that may have captured the microphone", "details": "If Zoom and Teams are both open, one may have locked the microphone. Close whichever app you are NOT using for this call."},
            {"step_number": 5, "instruction": "Update audio drivers via Device Manager", "details": "Right-click Start > Device Manager > Sound, video and game controllers > right-click your audio device > Update driver > Search automatically. Restart after updating."},
        ],
        "validation_steps": [{"step_number": 1, "instruction": "Ask the other participant to confirm they can hear you clearly."}],
    ...
    },
    {
        ...
        "keywords": ["no sound", "no audio", "speakers not working", "can't hear anything", "sound not working", "muted"],
        "ownership_group": "endpoint-productivity",
        "symptoms": ["No sound from laptop speakers or headset", "Volume slider present but no audio plays"],
        "probable_causes": ["Volume muted or at zero", "Wrong output device selected", "Audio driver issue"],
        "resolution_steps": [
            {"step_number": 1, "instruction": "Check volume level and ensure the laptop is not muted", "details": "Click the speaker icon in the taskbar. Make sure the slider is not at zero and the icon doesn't show a red X. Also check any physical volume/mute keys on your keyboard (Fn + mute key)."},
            {"step_number": 2, "instruction": "Right-click the speaker icon and select the correct output device", "details": "Right-click the speaker icon > Sound settings. Under Output, change the device to your speakers or headset. If a headset is plugged in, make sure Windows switched to it."},
            {"step_number": 3, "instruction": "Run the Windows Audio Troubleshooter", "details": "Settings > System > Troubleshoot > Other troubleshooters > Playing Audio > Run. Apply any recommendations."},
            {"step_number": 4, "instruction": "Restart the Windows Audio service", "details": "Press Win+R, type services.msc, find 'Windows Audio', right-click > Restart. Also restart 'Windows Audio Endpoint Builder'."},
        ],
        "validation_steps": [{"step_number": 1, "instruction": "Play a short audio file or YouTube video and confirm sound comes through."}],
    ...
    # ── Hardware camera ────────────────────────────────────────────
    {
        ...
        "keywords": ["camera not working", "webcam not working", "black screen camera", "camera not detected", "no camera"],
        "ownership_group": "endpoint-productivity",
        "symptoms": ["Camera shows black screen in Zoom or Teams", "Camera not detected or missing from device list"],
        "probable_causes": ["Physical privacy shutter closed", "Windows camera privacy setting disabled", "Another app holding the camera"],
        "resolution_steps": [
            {"step_number": 1, "instruction": "Restart your laptop", "details": "A simple restart resolves many camera driver issues."},
            {"step_number": 2, "instruction": "Check if camera is physically blocked", "details": "Many laptops have a physical privacy shutter. Ensure it is open/slid to reveal the camera lens."},
            {"step_number": 3, "instruction": "Enable Camera Access in Privacy & Security settings", "details": "Go to Settings > Privacy & Security > Camera. Ensure 'Camera access' is turned ON."},
            {"step_number": 4, "instruction": "Enable camera permissions for specific apps", "details": "In the same Camera settings, ensure the apps you need (Zoom, Teams) have camera permission enabled."},
            {"step_number": 5, "instruction": "Close other applications using the camera", "details": "Only one app can use the camera at a time. Close Teams if using Zoom, and vice versa."},
            {"step_number": 6, "instruction": "Test camera in the application's settings", "details": "Open Zoom/Teams Settings > Video/Camera and check if you see your preview."},
            {"step_number": 7, "instruction": "Check Device Manager for camera driver", "details": "Right-click Start > Device Manager > Cameras. Check for warning icons. Right-click > Update driver if needed."},
        ],
        "validation_steps": [{"step_number": 1, "instruction": "Open Zoom or Teams settings and verify you see your own camera preview."}],
    ...
    # ── Network connectivity ───────────────────────────────────────
    {
        ...
        "keywords": ["vpn not connecting", "vpn won't connect", "can't connect to vpn", "vpn failed", "globalprotect", "cisco vpn"],
        "ownership_group": "network-access",
        ...
        "resolution_steps": [
            {"step_number": 1, "instruction": "Disconnect from VPN completely and close the client", "details": "Right-click the VPN icon in the system tray and choose Disconnect. Then right-click again and Exit/Quit the application entirely."},
            {"step_number": 2, "instruction": "Restart your Wi-Fi or switch to a wired connection", "details": "Turn Wi-Fi off and on again on your laptop, or plug in an Ethernet cable if available."},
            {"step_number": 3, "instruction": "Reopen the VPN client and connect to the Aditi gateway", "details": "Open GlobalProtect or Cisco AnyConnect. Ensure the gateway address is set to the Aditi VPN endpoint. Click Connect."},
            {"step_number": 4, "instruction": "Authenticate with your Aditi email and password + MFA", "details": "Enter your @aditiconsulting.com email and current password. Approve the MFA push notification or enter the 6-digit Authenticator code."},
            {"step_number": 5, "instruction": "Restart your laptop if VPN still won't connect", "details": "A full restart clears stale network sessions. After reboot, try connecting to VPN before opening any other applications."},
        ],
        "validation_steps": [{"step_number": 1, "instruction": "Confirm VPN shows 'Connected' and access an internal resource (e.g. intranet page)."}],
    ...
    },
    {
        ...
        "keywords": ["wifi keeps dropping", "wi-fi disconnecting", "slow internet", "keeps disconnecting", "unstable connection"],
        "ownership_group": "network-access",
        "symptoms": ["Wi-Fi disconnects repeatedly", "Internet connection is slow or intermittent"],
        "probable_causes": ["Wi-Fi adapter power management switching off the adapter", "Too far from access point", "Outdated Wi-Fi driver"],
        "resolution_steps": [
            {"step_number": 1, "instruction": "Disable Wi-Fi adapter power management", "details": "Right-click Start > Device Manager > Network Adapters > right-click your Wi-Fi adapter > Properties > Power Management tab > uncheck 'Allow the computer to turn off this device to save power'."},
            {"step_number": 2, "instruction": "Forget and rejoin the Wi-Fi network", "details": "Click the Wi-Fi icon in the taskbar > click the network name > Forget. Then reconnect and enter the password."},
            {"step_number": 3, "instruction": "Run the Windows Network Troubleshooter", "details": "Settings > System > Troubleshoot > Other troubleshooters > Internet Connections > Run. Apply any fixes it suggests."},
            {"step_number": 4, "instruction": "Update the Wi-Fi driver", "details": "Device Manager > Network Adapters > right-click Wi-Fi adapter > Update driver > Search automatically. Restart after updating."},
            {"step_number": 5, "instruction": "Move closer to the Wi-Fi access point or switch to 5 GHz band", "details": "If the router broadcasts both 2.4 GHz and 5 GHz, connect to the 5 GHz network (usually labelled '_5G') for faster, more stable connection when in range."},
        ],
        "validation_steps": [{"step_number": 1, "instruction": "Stay connected for 15 minutes and confirm no drops."}],
    ...
    },
    {
        ...
        "issue_type": "connectivity",
        "severity_hint": "high",
        "tags": ["internet", "no internet", "can't connect", "connectivity", "offline", "disconnected"],
        "keywords": ["can't connect to internet", "no internet", "internet not working", "no network", "offline", "disconnected from internet"],
        "ownership_group": "network-access",
        "symptoms": ["No internet access on laptop", "All websites fail to load"],
        "probable_causes": ["Network adapter issue", "DHCP or DNS misconfiguration", "Router or gateway problem"],
        "resolution_steps": [
            {"step_number": 1, "instruction": "Check if you're connected to a network", "details": "Look at the Wi-Fi or Ethernet icon in the taskbar. If using a wired connection, make sure the Ethernet cable is plugged in."},
            {"step_number": 2, "instruction": "Disconnect and reconnect your network adapter", "details": "Right-click the Wi-Fi icon > Wi-Fi > turn Off. Wait 5 seconds, then turn it On again. Or, if wired, unplug the Ethernet cable for 10 seconds and plug it back in."},
            {"step_number": 3, "instruction": "Forget and rejoin the Wi-Fi network (if on Wi-Fi)", "details": "Click the Wi-Fi icon > click the network name > Forget. Then click the Wi-Fi icon again and select the same network."},
            {"step_number": 4, "instruction": "Try a different network to isolate the issue", "details": "Connect to a phone hotspot to confirm if the issue is your network or your laptop."},
            {"step_number": 5, "instruction": "Run the Windows Network Troubleshooter", "details": "Settings > System > Troubleshoot > Other troubleshooters > Internet Connections > Run."},
            {"step_number": 6, "instruction": "Restart your laptop", "details": "A full reboot clears network adapter drivers and DHCP lease issues."},
        ],
        "validation_steps": [{"step_number": 1, "instruction": "Open a browser and confirm websites load after the fix."}],
    ...
    # ── Access / permissions granular articles ─────────────────────
    {
        ...
        "keywords": ["account locked", "locked out", "can't login", "unable to login", "login failed", "too many attempts"],
        "ownership_group": "network-access",
        "symptoms": ["Account is locked — cannot log in to Windows, VPN, or Office 365", "Message says 'Your account has been locked'"],
        "probable_causes": ["Too many failed login attempts triggered lockout policy", "Saved password in an app retrying with an old password"],
        "resolution_steps": [
            {"step_number": 1, "instruction": "Stop all login attempts immediately", "details": "Each failed attempt resets the lockout timer. Do not try again until you follow the steps below."},
            {"step_number": 2, "instruction": "Use Self-Service Password Reset (SSPR) to unlock your account", "details": "Go to https://aka.ms/sspr in a browser. Enter your Aditi email, verify via registered method, and reset your password. This unlocks and resets in one step."},
            {"step_number": 3, "instruction": "Wait 30 minutes if SSPR is not available", "details": "If you cannot access SSPR, the account auto-unlocks after 30 minutes of no login attempts."},
            {"step_number": 4, "instruction": "Update saved passwords everywhere", "details": "After resetting, update the saved password in your browser, VPN client, Outlook mobile app, and any other apps that may be retrying with the old password."},
        ],
        "validation_steps": [{"step_number": 1, "instruction": "Log in to portal.office.com and confirm access to Outlook and Teams."}],
    ...
    },
    {
        ...
        "keywords": ["password expired", "must change password", "change your password", "password reset", "force change password"],
        "ownership_group": "network-access",
        "symptoms": ["Windows prompts for a password change on login", "Outlook or VPN stops working suddenly"],
        "probable_causes": ["Aditi's 90-day password policy has expired"],
        "resolution_steps": [
            {"step_number": 1, "instruction": "Change your password using Ctrl+Alt+Del on your Windows laptop", "details": "Press Ctrl+Alt+Del and select 'Change a password'. Enter the old password and set a new one (12+ characters, uppercase, lowercase, number, symbol)."},
            {"step_number": 2, "instruction": "Alternatively, use SSPR at https://aka.ms/sspr", "details": "If you cannot log in at all, go to https://aka.ms/sspr, verify your identity, and set a new password remotely."},
            {"step_number": 3, "instruction": "Update your password in all connected apps and devices", "details": "After changing: update Outlook mobile, VPN client (GlobalProtect/Cisco), Teams mobile, and any browser saved passwords. Old passwords stored in apps will re-lock your account."},
            {"step_number": 4, "instruction": "Re-authenticate in Outlook and VPN", "details": "Open Outlook — it may prompt for your new credentials. Open your VPN client and sign in again with the new password."},
        ],
        "validation_steps": [{"step_number": 1, "instruction": "Log in successfully and confirm VPN and Outlook are both working with the new password."}],
    ...
    },
    {
        ...
        "keywords": ["mfa not working", "authenticator not working", "no code", "two factor", "2fa", "push notification not arriving"],
        "ownership_group": "network-access",
        "symptoms": ["Microsoft Authenticator push notification not arriving", "6-digit codes not accepted", "Cannot complete MFA to sign in"],
        "probable_causes": ["Phone time not set to automatic", "Authenticator app needs a refresh", "Phone changed — MFA not re-registered"],
        "resolution_steps": [
            {"step_number": 1, "instruction": "Ensure your phone's date and time are set to automatic", "details": "Go to Settings > General (iOS) or Settings > Date & Time (Android) and enable 'Set Automatically'. An incorrect clock causes TOTP codes to be invalid."},
            {"step_number": 2, "instruction": "Open the Microsoft Authenticator app and pull down to refresh", "details": "Open Authenticator, find your Aditi account, and pull down to force a refresh."},
            {"step_number": 3, "instruction": "Use the 6-digit code instead of waiting for the push notification", "details": "Tap your Aditi account in Authenticator to see the 6-digit TOTP code. Enter this manually on the sign-in page when prompted."},
            {"step_number": 4, "instruction": "If you have lost your phone, use a backup method", "details": "On the Microsoft sign-in page, click 'Sign in another way' to use SMS, email, or a backup code."},
        ],
        "validation_steps": [{"step_number": 1, "instruction": "Sign in to portal.office.com and confirm MFA completes successfully."}],
    ...
    },
    {
        ...
        "issue_type": "provisioning",
        "severity_hint": "high",
        "tags": ["new joiner", "onboarding", "email not created", "laptop", "tools", "provisioning", "access"],
        "keywords": ["new joiner", "new employee", "just joined", "email not created", "account not set up", "laptop not received", "tools not provisioned"],
        "ownership_group": "endpoint-productivity",
        "symptoms": ["New employee does not have email or M365 access on day 1", "Laptop not received or not enrolled in Intune"],
        "probable_causes": ["IT New Joiner Request Form not submitted before start date"],
        "resolution_steps": [
            {"step_number": 1, "instruction": "Confirm the New Joiner Request Form was submitted before your start date", "details": "Your manager or HR should have submitted the IT New Joiner Request Form at least 3 business days before your start date."},
            {"step_number": 2, "instruction": "Raise an urgent IT ticket: 'New Joiner Access Missing — [Your Full Name]'", "details": "Include: full name, employee ID, start date, location, manager name and email, and the list of missing access. Mark the ticket as High priority."},
            {"step_number": 3, "instruction": "Ask your manager to CC themselves on the ticket", "details": "Manager confirmation speeds up provisioning. Your manager should also follow up directly with IT if day-1 access is critically needed."},
            {"step_number": 4, "instruction": "IT will provision email, Ruddr, and core tools within the agreed SLA", "details": "Once IT receives the complete ticket, email, Ruddr, and M365 access are provisioned within 1 business day. Laptop shipment follows a separate timeline."},
        ],
        "validation_steps": [{"step_number": 1, "instruction": "Log in to portal.office.com and confirm access to Outlook, Teams, and Company Portal."}],
    ...
    },
    {
        ...
        "issue_type": "provisioning",
        "severity_hint": "medium",
        "tags": ["ruddr", "account missing", "not created", "new joiner", "timesheet", "project tracking"],
        "keywords": ["ruddr", "ruddr account", "ruddr account missing", "ruddr not set up", "cannot access ruddr", "ruddr timesheet"],
        "ownership_group": "endpoint-productivity",
        "symptoms": ["Cannot log in to Ruddr at app.ruddr.io", "Account not found or access denied"],
        "probable_causes": ["Ruddr account not provisioned — onboarding ticket not raised", "New joiner form missing Ruddr in the request"],
        "resolution_steps": [
            {"step_number": 1, "instruction": "Raise an IT support ticket immediately", "details": "Go to Freshservice and raise a ticket titled: 'Ruddr Account Missing — [Your Full Name]'."},
            {"step_number": 2, "instruction": "Include your full name, employee ID, manager name, and start date in the ticket", "details": "IT needs these details to locate your record and create or restore your Ruddr account."},
            {"step_number": 3, "instruction": "CC your manager on the ticket", "details": "Ruddr access is linked to your project assignment. Your manager must confirm the project code."},
            {"step_number": 4, "instruction": "Wait up to 1 business day for IT to action the ticket", "details": "Once IT receives the ticket with all required information, your Ruddr account will be created within 1 business day."},
        ],
        "validation_steps": [{"step_number": 1, "instruction": "Log in to app.ruddr.io and confirm your name, team, and assigned projects appear."}],
    ...
    },
    {
        ...
        "keywords": ["ruddr account locked", "ruddr account disabled", "ruddr blocked", "ruddr access denied", "ruddr suspended"],
        "ownership_group": "endpoint-productivity",
        "symptoms": ["Ruddr login returns 'Account disabled' or 'Access denied'"],
        "probable_causes": ["Account disabled during extended leave or audit", "Account suspended by admin"],
        "resolution_steps": [
            {"step_number": 1, "instruction": "Do not attempt repeated logins — raise an IT ticket instead", "details": "Multiple failed login attempts will not help. This issue requires IT intervention."},
            {"step_number": 2, "instruction": "Raise a ticket: 'Ruddr Account Disabled — [Your Full Name]'", "details": "Include your full name, employee ID, your manager's name, and the date you last had access."},
            {"step_number": 3, "instruction": "CC your manager on the ticket", "details": "Your manager's confirmation is required before IT re-enables access — this is a security control."},
        ],
        "validation_steps": [{"step_number": 1, "instruction": "Log in to app.ruddr.io and confirm access is restored."}],
    ...
    },
    {
        ...
        "issue_type": "provisioning",
        "severity_hint": "low",
        "tags": ["license", "tool access", "copilot", "github", "linkedin recruiter", "keeper", "software"],
        "keywords": ["license request", "software license", "tool access", "copilot license", "github access", "linkedin recruiter", "keeper"],
        "ownership_group": "endpoint-productivity",
        "symptoms": ["Need access to Microsoft Copilot, GitHub Copilot, LinkedIn Recruiter, or Keeper", "License not assigned to my account"],
        "probable_causes": ["License not yet assigned — requires manager approval before provisioning"],
        "resolution_steps": [
            {"step_number": 1, "instruction": "Get verbal or written approval from your manager before raising a ticket", "details": "All tool licenses require manager sign-off. Confirm the business need before raising the IT ticket."},
            {"step_number": 2, "instruction": "Raise an IT ticket: 'License Request — [Tool Name] — [Your Name]'", "details": "Include: tool/software name, business justification, project name (if applicable), manager name and approval confirmation."},
            {"step_number": 3, "instruction": "IT will confirm with procurement and provision within 1–3 business days", "details": "Once approved, IT will provision access. You will receive an email with login instructions."},
            {"step_number": 4, "instruction": "For GitHub access — include the repo or org name", "details": "If you need access to a specific Aditi GitHub organisation or repository, include the exact repo URL in the ticket."},
        ],
        "validation_steps": [{"step_number": 1, "instruction": "Open the requested application and confirm the licensed features are available (e.g. Copilot icon in Word, Copilot chat in VS Code)."}],
    ...
    },
    {
        ...
        "severity_hint": "medium",
        "tags": ["license not working", "access denied", "tool not working", "software error", "provisioned but blocked"],
        "keywords": ["access denied", "license not working", "tool not working after provisioning", "not authorized", "licence expired"],
        "ownership_group": "endpoint-productivity",
        "symptoms": ["Tool shows 'Access Denied' or 'Not Authorized' after IT provisioned the license"],
        "probable_causes": ["Permissions not yet propagated (can take up to 60 min)", "Old browser session caching previous permissions"],
        "resolution_steps": [
            {"step_number": 1, "instruction": "Sign out of the tool completely and sign back in", "details": "Sign out from all instances (browser, desktop app, mobile) and sign back in with your Aditi SSO credentials."},
            {"step_number": 2, "instruction": "Clear browser cache and cookies for the tool's domain", "details": "In Chrome: Ctrl+Shift+Delete > Cached images and files + Cookies > Clear data. Then reopen the tool's website and sign in fresh."},
            {"step_number": 3, "instruction": "Wait up to 1 hour for the license assignment to propagate", "details": "M365 and most SaaS tools take up to 60 minutes for new license assignments to become active."},
            {"step_number": 4, "instruction": "Try accessing from a private/incognito browser window", "details": "Open an incognito window (Ctrl+Shift+N) and visit the tool. If it works in incognito, the issue is a cached session — clearing cookies will fix it permanently."},
        ],
        "validation_steps": [{"step_number": 1, "instruction": "Confirm the licensed features are accessible (e.g. Copilot icon appears, GitHub shows organisation membership)."}],
    ...
    # ── Sixth Sense / Naukri ───────────────────────────────────────
    {
        ...
        "keywords": ["sixth sense login", "naukri login", "sixth sense blocked", "naukri account locked", "unhandled message"],
        "ownership_group": "network-access",
        "symptoms": ["Cannot log in to Sixth Sense or Naukri", "Message says account is blocked after multiple failed attempts"],
        "probable_causes": ["5+ incorrect password attempts triggered lockout", "Direct naukri.com login causing Unhandled Message error"],
        "resolution_steps": [
            {"step_number": 1, "instruction": "Stop all login attempts immediately", "details": "Each attempt resets the 1-hour lock timer. Do NOT try to log in again."},
            {"step_number": 2, "instruction": "Wait for 1 hour for the account to auto-unlock", "details": "The Naukri system automatically unlocks your account after 1 hour of inactivity."},
            {"step_number": 3, "instruction": "After 1 hour, reset your password at naukri.com", "details": "Visit naukri.com and click 'Forgot Password'. Follow the email/OTP verification to set a new password."},
            {"step_number": 4, "instruction": "Update the new password in the Sixth Sense portal", "details": "Once your Naukri password is reset, open the Sixth Sense portal and update your credentials."},
            {"step_number": 5, "instruction": "Always use the Sixth Sense portal URL, not naukri.com directly", "details": "Direct login via naukri.com can trigger the 'Unhandled Message' error. Use the portal URL provided by IT."},
        ],
        "validation_steps": [{"step_number": 1, "instruction": "Log in to the Sixth Sense portal and confirm you can search and view candidate profiles."}],
    ...
    },
    {
        ...
        "keywords": ["unhandled message", "sixth sense error", "naukri error", "portal login", "sixth sense unhandled"],
        "ownership_group": "network-access",
        "symptoms": ["'Unhandled Message' error when logging in to Naukri or Sixth Sense"],
        "probable_causes": ["User tried to log in directly via naukri.com instead of the Sixth Sense portal"],
        "resolution_steps": [
            {"step_number": 1, "instruction": "Do not log in through naukri.com directly", "details": "Direct login via the Naukri website triggers the 'Unhandled Message' error due to SSO/integration configuration."},
            {"step_number": 2, "instruction": "Use the Sixth Sense portal URL provided by IT", "details": "Always access Sixth Sense through the designated portal URL. This ensures proper authentication flow."},
            {"step_number": 3, "instruction": "Clear browser cache and cookies if the error persists", "details": "Press Ctrl+Shift+Delete, clear cookies for naukri.com and the Sixth Sense portal domain, then try again."},
        ],
        "validation_steps": [{"step_number": 1, "instruction": "Log in via the portal URL and confirm you reach the Sixth Sense dashboard."}],
    ...
    },
    {
        ...
        "keywords": ["otp not received", "sixth sense otp", "naukri otp", "no otp", "verification code not arriving"],
        "ownership_group": "network-access",
        ...
        "resolution_steps": [
            {"step_number": 1, "instruction": "Ensure the 1-hour lock period has fully elapsed", "details": "OTP delivery may be blocked during the active lock period. Wait the full hour before requesting any OTP."},
            {"step_number": 2, "instruction": "Check spam/junk folder in your registered email", "details": "OTP emails from Naukri sometimes land in spam. Search for emails from naukri.com in your junk folder."},
            {"step_number": 3, "instruction": "Request a new OTP after a 2-minute wait", "details": "If the first OTP didn't arrive, wait at least 2 minutes before requesting a new one. Rapid requests may be rate-limited."},
            {"step_number": 4, "instruction": "Verify your registered email/phone is correct", "details": "If you recently changed your email or phone, the OTP may be going to the old contact. Contact IT to update your registered details."},
        ],
        "validation_steps": [{"step_number": 1, "instruction": "Enter the OTP successfully and confirm you can complete the password reset."}],
    ...
    # ── Zoom granular articles ─────────────────────────────────────
    {
        ...
        "keywords": ["zoom sign in", "zoom login", "zoom SSO", "zoom not signing in", "zoom authentication"],
        "ownership_group": "endpoint-productivity",
        "symptoms": ["Cannot sign in to Zoom", "Zoom login fails or shows error"],
        "probable_causes": ["Using email/password instead of SSO", "Stale Zoom cache", "KYC not completed for India-based users"],
        "resolution_steps": [
            {"step_number": 1, "instruction": "Open Zoom and click 'Sign In with SSO'", "details": "Do NOT use regular email/password login. SSO is required for Aditi accounts."},
            {"step_number": 2, "instruction": "Enter the company SSO domain: aditiconsulting", "details": "Type 'aditiconsulting' when prompted for the company domain, then complete authentication."},
            {"step_number": 3, "instruction": "Complete authentication via the Aditi login page", "details": "Use your Aditi corporate credentials (email + MFA) to authenticate on the Microsoft login page."},
            {"step_number": 4, "instruction": "For India-based users: complete KYC if prompted", "details": "Indian regulatory requirements may require KYC completion for Zoom access."},
            {"step_number": 5, "instruction": "If SSO fails, clear Zoom cache", "details": "Close Zoom > Delete %AppData%/Zoom folder > Restart Zoom and try SSO login again."},
        ],
        "validation_steps": [{"step_number": 1, "instruction": "Confirm you reach the Zoom home screen with your Aditi name and email shown."}],
    ...
    },
    {
        ...
        "keywords": ["zoom audio not working", "zoom microphone", "zoom speaker", "zoom no sound", "zoom echo"],
        "ownership_group": "endpoint-productivity",
        "symptoms": ["Cannot hear others in Zoom", "Others cannot hear me in Zoom", "Echo in Zoom calls"],
        "probable_causes": ["Wrong audio device selected in Zoom", "Another app locking the microphone or speaker"],
        "resolution_steps": [
            {"step_number": 1, "instruction": "Open Zoom Settings > Audio", "details": "Click your profile icon (top right) > Settings > Audio."},
            {"step_number": 2, "instruction": "Verify correct speaker device is selected", "details": "Click 'Test Speaker' to confirm you can hear the test tone. Select the correct device from the dropdown."},
            {"step_number": 3, "instruction": "Verify correct microphone device is selected", "details": "Click 'Test Mic' and speak — verify the input level bar moves. Select the correct device."},
            {"step_number": 4, "instruction": "Check system audio settings", "details": "Open Windows Sound Settings and ensure correct default devices are set for both input and output."},
            {"step_number": 5, "instruction": "Close other applications that may use audio", "details": "Close Teams, Discord, or other apps that might lock audio devices. Restart Zoom."},
        ],
        "validation_steps": [{"step_number": 1, "instruction": "Use Zoom's Test Speaker and Test Microphone and confirm both work before joining the call."}],
