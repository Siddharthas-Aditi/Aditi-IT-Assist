...
_OUTLOOK_RULES: tuple[SubtypeRule, ...] = (
    SubtypeRule(
        ...
        keywords=(
            "inbox is full", "inbox full", "mailbox is full", "mailbox full",
            "mailbox storage", "storage full", "storage is full", "out of space",
            "no space", "over quota", "quota", "over the limit", "over limit",
            "exceeded", "mailbox size", "mailbox is almost full", "almost full",
            "reduce mailbox", "clean up mailbox", "cleanup mailbox", "free up space",
            "98%", "99%", "100% full",
        ),
    ...
    SubtypeRule(
        ...
        keywords=(
            "not receiving", "no new emails", "not getting emails",
            "not getting any emails", "emails not coming", "stopped receiving",
            "missing emails", "missing email", "no emails", "no incoming",
            "not coming through", "emails not arriving", "not arriving",
        ),
    ...
    SubtypeRule(
        ...
        keywords=(
            "work offline", "working offline", "offline mode", "shows offline",
            "says offline", "disconnected from server", "outlook is offline",
        ),
    ),
    SubtypeRule(
        subtype="outlook-crash",
        keywords=(
            "crash", "crashing", "keeps closing", "keeps crashing",
            "won't open", "will not open", "not opening", "doesn't open",
            "closes by itself", "shuts down",
        ),
    ),
    SubtypeRule(
        subtype="outlook-slow",
        keywords=(
            "slow", "sluggish", "lagging", "laggy", "freezes", "freezing",
            "hangs", "hanging", "not responding", "takes forever", "very slow",
        ),
    ),
    SubtypeRule(
        subtype="calendar-sync",
        keywords=(
            "calendar", "meeting not showing", "appointment not showing",
            "calendar not syncing", "meetings missing", "invites not showing",
        ),
    ),
    SubtypeRule(
        subtype="search-not-working",
        keywords=(
            "search not working", "can't search", "cannot search",
            "search isn't", "search returns nothing", "search broken",
        ),
    ),
    SubtypeRule(
        subtype="rule-issue",
        keywords=(
            "mail rule", "email rule", "rules moving", "rule is moving",
            "auto move", "auto-move", "filter moving",
        ),
    ),
    SubtypeRule(
        subtype="addin-issue",
    ...
    SubtypeRule(
        ...
        keywords=(
            "can't sign in", "cannot sign in", "can't log in", "cannot log in",
            "sign in", "sign-in", "signin", "login", "log in", "password prompt",
            "keeps asking for password", "authentication", "credentials",
        ),
    ),
)
...
_ACCESS_RULES: tuple[SubtypeRule, ...] = (
    SubtypeRule(
        ...
        keywords=(
            "account locked", "account is locked", "locked out", "blocked",
            "account blocked", "too many attempts", "temporarily locked",
        ),
    ),
    SubtypeRule(
        subtype="password-expired",
        keywords=(
            "password expired", "reset password", "reset my password",
    ...
    SubtypeRule(
        ...
        keywords=(
            "unable to login", "unable to log in", "can't login", "cannot login",
            "can't log in", "cannot log in", "login failed", "login issue",
            "login problem", "can't sign in", "cannot sign in", "sign in",
...
_ZOOM_RULES: tuple[SubtypeRule, ...] = (
    SubtypeRule(subtype="no-audio", keywords=("no audio", "can't hear", "cannot hear", "no sound", "mic not", "microphone")),
    SubtypeRule(subtype="no-video", keywords=("camera not", "no video", "video not working", "webcam")),
    SubtypeRule(subtype="cant-join-meeting", keywords=("can't join", "cannot join", "unable to join", "won't let me join")),
    SubtypeRule(subtype="screen-share-issue", keywords=("screen share", "screen sharing", "can't share")),
    SubtypeRule(subtype="poor-quality", keywords=("poor quality", "choppy", "laggy", "freezing", "buffering")),
...
_INTUNE_RULES: tuple[SubtypeRule, ...] = (
    SubtypeRule(subtype="non-compliant", keywords=("non-compliant", "not compliant", "compliance", "device not compliant")),
    SubtypeRule(subtype="enrollment-failure", keywords=("enroll", "enrollment", "can't enroll", "enrollment failed")),
    SubtypeRule(subtype="sync-issue", keywords=("not syncing", "won't sync", "sync failed")),
    SubtypeRule(subtype="app-deployment", keywords=("app won't install", "app not installing", "company portal app")),
    SubtypeRule(subtype="conditional-access-blocked", keywords=("blocked from", "conditional access", "can't access office")),
)

_CAMERA_RULES: tuple[SubtypeRule, ...] = (
    SubtypeRule(subtype="camera-black-screen", keywords=("black screen", "no image", "blank")),
    SubtypeRule(subtype="camera-not-detected", keywords=("not detected", "no camera found", "camera missing")),
    SubtypeRule(subtype="camera-permission-denied", keywords=("permission denied", "no permission", "blocked")),
    SubtypeRule(subtype="camera-in-use-by-another-app", keywords=("in use", "another app", "already in use")),
    SubtypeRule(subtype="camera-poor-quality", keywords=("blurry", "poor quality", "grainy")),
)

_NETWORK_RULES: tuple[SubtypeRule, ...] = (
    SubtypeRule(subtype="vpn-not-connecting", keywords=("vpn won't connect", "vpn not connecting", "can't connect to vpn", "vpn failed")),
    SubtypeRule(subtype="wifi-disconnecting", keywords=("wifi", "wi-fi", "keeps dropping", "disconnecting")),
    SubtypeRule(subtype="internet-slow", keywords=("internet slow", "slow internet", "slow connection")),
    SubtypeRule(subtype="specific-site-unreachable", keywords=("can't reach", "site not loading", "page won't load")),
