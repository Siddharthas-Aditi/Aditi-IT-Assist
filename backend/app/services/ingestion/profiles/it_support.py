...
IT_SUPPORT_PROFILE = ParserProfile(
    name="it_support_v1",
    version="1.2.0",
    description="General IT support documents: SOPs, KB articles, ticket exports, onboarding guides.",
    ...
    section_markers={
        "symptoms": [
            "symptoms", "symptom", "signs", "indicators", "observable",
            "issue", "problem", "problem description", "user report",
            "reported issue", "what the user sees", "error", "errors",
        ],
        "probable_causes": [
            "cause", "causes", "root cause", "reason", "reasons",
            "probable cause", "likely cause", "why",
        ],
        "troubleshooting_steps": [
            "troubleshooting", "troubleshoot", "troubleshooting steps",
            "diagnosis", "diagnostic steps", "investigate", "investigation",
            "check", "checks", "verify", "verification steps",
            "initial steps", "first steps", "investigation steps",
        ],
        "resolution_steps": [
            "resolution", "solution", "fix", "resolve",
            "steps to resolve", "steps to fix", "how to fix",
            "corrective action", "action", "actions",
            "recommended fix", "permanent fix", "workaround",
            "steps", "procedure", "instructions",
        ],
        "validation_steps": [
            "validation", "verify fix", "confirm", "confirmation",
            "test", "testing", "post-fix check", "sanity check",
        ],
        "escalation_criteria": [
            "escalate", "escalation", "escalation criteria",
            "if the issue persists", "if unresolved",
            "contact", "reach out", "contact it", "contact support",
            "when to escalate", "further assistance",
        ],
    },

    # ── Heading detection extras (on top of universal heuristics) ─────────────
    heading_signals=[
        re.compile(r"^(?:issue|problem|topic|article|kb|note|important|warning|tip)\s*[:\-–]", re.I),
    ...
    detection=DetectionCriteria(
        keyword_signals=[
            "outlook", "zoom", "vpn", "intune", "sharepoint", "teams",
            "resolution", "symptoms", "troubleshoot", "escalate",
