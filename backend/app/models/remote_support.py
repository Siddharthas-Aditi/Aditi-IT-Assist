...
class RemoteSupportSession(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    provider: Mapped[str] = mapped_column(
        String(100), default="microsoft_remote_help",
...
class RemoteSupportConsent(UUIDPrimaryKeyMixin, Base):
    ...
    session_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("remote_support_sessions.id", ondelete="CASCADE"),
        index=True,
    )
    employee_id: Mapped[uuid.UUID] = mapped_column(
    ...
    granted: Mapped[bool] = mapped_column(
        Boolean, comment="True = granted, False = denied",
    )
