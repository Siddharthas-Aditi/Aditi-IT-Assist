...
### Autonomous Device Execution (Phase 9 — behind `FEATURE_DEVICE_EXECUTION`, default off)
- Lets the agent **act on an Intune-managed endpoint** — install an approved app (Python, Docker Desktop, …), run an approved remediation, or take a benign device action — autonomously for low-risk, human-approved above threshold. This narrows the Phase-1 remote-support ADR's "AI must never initiate desktop control" absolute to a **bounded, catalog-limited** surface (interactive Remote Help stays human-only). ADR: `docs/architecture/device-execution-decision.md`.
- **Keystone guardrail — no free-form execution.** The agent's only lever is a **catalog id**; it can never author an installer, script body, or command. `app/services/agents/device_actions/catalog.py` (`CATALOG_VERSION`): enumerated `APP_CATALOG`/`REMEDIATION_CATALOG`/`DEVICE_ACTION_CATALOG`, each id → a pre-published Intune object + `RiskTier`. This is the primary prompt-injection defense (no surface for injected text to run).
- **Autonomy policy** — pure `app/services/agents/device_actions/policy.py` (`AUTONOMY_POLICY_VERSION`): `evaluate_device_action` → `autonomous | human_approval | deny`. Off-catalog / no-device / ineligible / no-consent ⇒ deny; kill-switch or injection-scan hit ⇒ human_approval; **HIGH risk never autonomous**, MEDIUM only when opted in, LOW autonomous. `scan_for_injection` is a tripwire that only *downgrades* autonomy, never expands it.
- **Tools** — `app/services/agents/device_actions/tools.py`: `install_win32_app`, `run_remediation_script`, `device_action` (typed `WRITE`, perm `integration:device_execute` = it_lead+, `approval=AUTO_ALLOWLISTED` so the runtime lets `run()` apply the finer catalog+policy gate). Args are catalog id + device id + idempotency key + (scanned) justification — never a payload. Off-catalog/high-risk/suspicious ⇒ typed non-executing `denied`/`needs_approval` result, no Intune call.
- **MCP surface** — dedicated `msgraph_intune_exec` server profile (own enablement/allow-list/audit boundary; `MCP_PROFILE_VERSION` → 1.1.0), separate from read-only `msgraph`. Mock responses in `mock_session.py`. **Two gates**: build (`FEATURE_DEVICE_EXECUTION`) + autonomy kill-switch (`DEVICE_EXECUTION_AUTONOMOUS`; off ⇒ everything → human approval). Prod fail-fast if feature on with `MCP_USE_MOCK=true`.
- **Orchestrator (end-to-end)** — `app/services/agents/device_actions/service.py` `DeviceExecutionService`: request → guardrails → policy → route. `deny` ⇒ nothing dispatched; `autonomous` ⇒ mint a **scoped approval token** and dispatch (RBAC still enforced); `human_approval` ⇒ `ApprovalQueue.propose` (parked in the shared `/agent-ops/approvals` queue). `approve()` re-checks consent+eligibility, then `execute_approved`. Honours the `DEVICE_EXECUTION_AUTONOMOUS` kill-switch in routing. Unauthorized requester of an autonomous-eligible action falls back to the queue (0 unauthorized executions).
- **Guardrails (real seam, tool-layer enforced)** — `device_actions/guardrails.py` `DeviceGuardrails`: eligibility via the Intune compliance MCP read; consent via a `RemoteSupportConsent` lookup keyed on the device's **server-resolved owner** (Intune primary user — not a caller-supplied name). Enforced inside `DeviceExecutionTool.run` (`as_tool_provider`), so consent is re-checked on **every** path (autonomous, device approve, and the generic `/agent-ops` queue). Permissive only under `MCP_USE_MOCK`; DB-authoritative in prod.
- **API** — `app/api/v1/device_execution.py` (`/device-execution`): `GET /catalog` (it_agent+), `POST /actions` (it_agent+; routes to auto/queue/deny), `POST /approvals/{id}/approve|reject` (it_lead+, consent re-checked). Schemas `app/schemas/device_execution.py`.
- **Frontend** — `frontend/src/features/device-execution/` (typed API + React Query hooks) + **Device Actions** page (`/operations/device-actions`): catalog picker, request form, typed outcome (executed/queued/denied + risk tier + policy signals). Queued actions appear in the existing Approval Queue. `tsc` + `eslint` clean.
- **Wiring** — `build_default_runtime(include_device_execution=True)` (also merged into the shared `ApprovalQueue` runtime); device tools are `approval=human` so the runtime never executes without a token; `device_intune` specialist `allowed_tools` extended; `TOOL_REGISTRY_VERSION` → 1.1.0, `REGISTRY_VERSION` → 1.3.0. Every dispatch audited with arg/result hashes; result carries decision + risk tier + policy signals.
- **Improvement loop stays human-reviewed** — KB/vetted knowledge informs *which* catalog action to pick; genuinely new fixes become human-reviewed catalog entries (never live-authored code).
- Eval/gate: `tests/data/device_execution_safety_eval.yaml` + `test_device_execution_safety_eval.py` (**0 autonomous above threshold**, **0 off-catalog execution**, contract pins, dispatch gates); `test_device_execution_service.py` (full routing + approve path); `test_device_actions.py` (catalog/policy/injection). Docs: `docs/architecture/device-execution.md`; plan `plans/autonomous-device-execution.md`.
...
### Chat Session Store (persistence seam — hardening 2026-07)
- Multi-turn chat state, the escalation-ticket **idempotency** record, and the waiting-for-specialist timestamp live in a `SessionStore` (`app/services/agents/session_store.py`), NOT process-local dicts. This fixed three P1s: **IDOR** (sessions are now owner-bound — `ChatService._load_owned` rejects a caller who isn't the owner), **unbounded growth / multi-worker context loss** (bounded LRU + TTL in-memory; Redis shares state across workers), and **non-durable idempotency** (duplicate tickets on restart / double-click).
- Two backends behind one async protocol: `InMemorySessionStore` (default, `CHAT_SESSION_BACKEND=memory`) and `RedisSessionStore` (`=redis`, JSON envelope, degrades to in-memory on Redis outage). **Use `redis` for any multi-worker/production deploy.** `ChatSession` envelope carries `user_id`/`state`/`ticket`/`waiting_since`. LangChain messages (de)serialize via `messages_to_dict`/`from_dict`.
- Session state is only persisted **after a successful turn** (a failed `ainvoke` never leaves a half-updated session). Tests inject a store via `set_session_store(...)`. Tests: `test_session_store.py`, `test_chat_session_ownership.py`.

### Reliability/Security hardening pass (2026-07)
- **Lint/format gate is now enforced and green** — ruff is pinned (`>=0.15,<0.16`) with a documented ruleset (framework-idiomatic ignores: `B008` for FastAPI `Depends`, `N802` for config props, `UP042`/`N818`; `app/models/**` + `app/schemas/**` keep runtime type imports for SQLAlchemy/Pydantic). Run `uv run ruff check . && uv run ruff format --check .`.
- **Rate-limit client identity** uses the trusted-proxy-hop (`RATE_LIMIT_TRUSTED_PROXY_HOPS`, rightmost XFF), not the spoofable leftmost hop (brute-force-guard bypass fix).
- **Remote-session status polling** enforces the same participant/role visibility as `GET /sessions/{id}` (IDOR fix). **Idle sweeper** now commits warning transitions (`sweep_idle` returns `(ended, warned)`). **`start()`** uses a SAVEPOINT (no caller-transaction rollback). **Remote sweeper** also expires stale `consent_granted`/`connecting` sessions; late-consent expiry is committed before raising. `resolve()`/`release()` end the linked live chat. `end()` + sweeper use row locks (`with_for_update`).
- **Approval queue** claims a record (`EXECUTING`) before awaiting execution (TOCTOU double-execute fix). **Device execution** honours `FEATURE_DEVICE_EXECUTION` in the service + approve route (not just `/actions`).
- **Config `validate_production()`** now rejects non-https/`*` CORS origins and local auth under SSO (`LOCAL_AUTH_ENABLED`). `/auth/register` + local `/auth/login` are gated by `LOCAL_AUTH_ENABLED`. SAML `validate_session` enforces refresh-reject + denylist parity with local.
- **Frontend**: React error boundaries (root + per-layout) prevent white-screens; React Query no longer retries 4xx; chat surfaces typed API errors.
- **Docker/infra**: reproducible builds (`uv sync --frozen`, `npm ci`), readiness-gated healthcheck (`/health/ready`), nginx re-resolves the backend hostname. `.env.example` `VITE_API_URL` includes `/api/v1`; `CORS_ORIGINS` documented.
- **Workflow**: `audit_trail` accumulates per-turn (reducer) instead of being overwritten; context summarizer rehydrates `DiagnosticContext` (was dead code); resolved users aren't force-escalated at max turns; sentiment urgency matches on word boundaries.
